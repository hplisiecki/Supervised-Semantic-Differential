...
def _validate_cols(cols, allowed: Sequence[str]) -> tuple[tuple[str, ...], str | None]:
...
def _check_style(ext: str, style: str | None) -> None:
...
class View(Generic[T]):
    ...
    def to_html(self, cols: Sequence[str] | None = None) -> str:
    ...
    # -------- unified save() ---------------------------------------------
    def save(self, path: str, *, cols: Sequence[str] | None = None,
             k: int | None = None, style: str | None = None) -> None:
        ...
        style : str, optional
            Document style — only valid for ``.docx``. Raises ``TypeError``
            on any other extension.
        """
        ext = _get_ext(path)
        _check_ext(ext)
        _check_style(ext, style)

        view = self._resized(k) if k is not None else self
        keep, warning = _validate_cols(cols, view._columns)
        if warning:
            _warn(warning)
    ...
    def to_text(self, max_rows: int = 20, cols: Sequence[str] | None = None) -> str:
...
class ScalarView(View):
    ...
    def to_dict(self, cols: Sequence[str] | None = None) -> dict:
...
class TestView(ScalarView):
    ...
    def _rerun_hint(self) -> str | None:
        """Subclasses return a 'Rerun: .test(...)' line or None."""
        return None
...
class Result(ABC):
    ...
    def _access_hint(self) -> str:
        """Render the views/methods access list."""
        if not self._access:
            return ""
        views: list[str] = []
        methods: list[str] = []
        for name in self._access:
            try:
                attr = getattr(self, name.split("(")[0], None)
            except Exception:
                attr = None
            if callable(attr) or "(" in name:
                methods.append(name)
            else:
                views.append(name)
        lines = []
    ...
    def to_text(self) -> str:
        """Compact text — used by __repr__, no citation."""
        parts = [self._summary()]
    ...
    def to_html(self) -> str:
        """Compact HTML — used by _repr_html_, no citation."""
